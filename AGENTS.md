...
- Prefer `./script/build_and_run.sh --verify` for app build validation.
- Use focused smoke tests in `script/` when changing user interaction.
- Add or update UI smoke tests when board interaction, persistence, sizing, new-board behavior, or visual progress behavior changes.
- When UI automation is flaky because of platform accessibility focus, keep tests focused on externally observable behavior and document the practical limitation in the increment review.
...
- `Reviewer` prioritizes correctness, regressions, missing tests, user-interaction failures, persistence safety, accessibility/testability risks, and maintainability.

For AIM work, every Done Increment that touches app code must include a short architecture note in its review.
