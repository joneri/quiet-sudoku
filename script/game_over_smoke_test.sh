...
STATE_FILE="$(mktemp "${TMPDIR:-/tmp}/StillgridSudoku-game-over-state.XXXXXX.json")"
SAVE_FILE="$(mktemp "${TMPDIR:-/tmp}/StillgridSudoku-game-over-save.XXXXXX.json")"
LEADERBOARD_FILE="$(mktemp "${TMPDIR:-/tmp}/StillgridSudoku-leaderboard.XXXXXX.json")"
